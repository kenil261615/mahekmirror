...
if os.path.exists('cc_scrapper.txt'):
    with open('cc_scrapper.txt', 'r+') as f:
        f.truncate(0)
...
try:
    CHROMEDRIVER_PATH = getConfig('CHROMEDRIVER_PATH')
    GOOGLE_CHROME_BIN = getConfig('GOOGLE_CHROME_BIN')
    if len(CHROMEDRIVER_PATH) == 0 or len(GOOGLE_CHROME_BIN) == 0:
        raise KeyError
except KeyError:
